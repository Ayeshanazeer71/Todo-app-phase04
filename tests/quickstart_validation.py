"""
Quickstart validation script to ensure the application works as expected
"""
import sys
import os
...
def validate_quickstart():
    """
    Validate that the application works as expected according to the specification
    """
    print("Validating Todo Console Application...")
    ...
    # Update task
    success = manager.update_task(task_id, title="Updated Task")
    ...
    # Toggle status
    success = manager.toggle_task_status(task_id)
    assert success == True, "Toggle should succeed"
    toggled_task = manager.get_task_by_id(task_id)
    ...
    manager1.add_task("Task 1", "Description 1")
    manager1.add_task("Task 2", "Description 2")

    # Create a new manager (simulating a new session)
    manager2 = TodoManager()
    assert len(manager2.list_tasks()) == 0, "New manager should have no tasks (in-memory only)"
    print("   [PASS] In-memory storage works correctly (no persistence)")

    # Test 4: Verify error handling
    print("\n4. Testing error handling...")
    # Try to update non-existent task
    success = manager.update_task(999, title="Non-existent")
    ...
    success = manager.toggle_task_status(999)
    assert success == False, "Toggle should fail for non-existent task"
    print("   [PASS] Error handling works correctly")

    print("\n[SUCCESS] All validation tests passed!")
    print("The Todo Console Application is working as expected.")
